<template>
	<view class="login_content">
		...
		<view class="main">
			...
			<view class="inputs">
				<input type="text" value="" placeholder="用户名/邮箱/手机号" class="user" v-model="form.user" />
				<input type="password" value="" placeholder="密码" class="password" v-model="form.password" />
			</view>
			<!-- <transition  name="xwl">
						...
						success:(res)=>{
							...
							if (res.data.status==200) {
								...
								try {
									...
									uni.navigateTo({
										url: '../index/index?id='+res.data.back.id+'&token='+this.token,
										success: res => {},
										fail: () => {},
										complete: () => {}
									});
